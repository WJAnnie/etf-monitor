...
def _sizing_row(*, signal_id: str = "sig-daily-2b", execution_signal_id: str = "sig-5m-2b") -> dict:
    return {
        "market": 1,
        "code": "600000",
        "name": "浦发银行",
            ...
            "allocations": [
                {
                    ...
                    "allocated_risk_cny": str(Decimal(quantity) * Decimal("0.5")),
                    "binding_limits": [],
                    "reason": "test",
                }
            ],
...
def test_no_allocation_needs_no_reservation_context():
    allocation = {
        "plan": {
            "state": "NO_ELIGIBLE",
            "snapshot_id": None,
            "allocations": [{"identity": "1:600000:STOCK", "state": "NOT_SIZED"}],
        }
    }
...
def test_matching_snapshot_builds_immutable_dual_stop_reservation_bundle():
    ...
    assert plan.allocated_risk_cny == Decimal("500")
    ...
    assert intent.structural_stop_price == Decimal("9.5")
    assert intent.authority_stop_price == Decimal("9.2")
...
def test_same_plan_has_same_idempotency_key_but_authority_or_execution_signal_change_changes_key():
    ...
    changed_authority = build_execution_reservation_plan(
        _allocation_payload(), [_sizing_row(signal_id="sig-daily-new")], _reservation_context()
    )
    changed_execution = build_execution_reservation_plan(
        _allocation_payload(), [_sizing_row(execution_signal_id="sig-5m-new")], _reservation_context()
    )
    assert first.reservation_key == second.reservation_key
    assert changed_authority.state is ReservationPlanState.READY_TO_RESERVE
    assert changed_execution.state is ReservationPlanState.READY_TO_RESERVE
    assert changed_authority.reservation_key != first.reservation_key
    assert changed_execution.reservation_key != first.reservation_key
...
def test_daily_authority_stop_identity_must_remain_bound_to_permission():
    sizing = deepcopy(_sizing_row())
    sizing["authority_stop"]["signal_id"] = "other-signal"
    plan = build_execution_reservation_plan(_allocation_payload(), [sizing], _reservation_context())
    assert plan.state is ReservationPlanState.CONTRACT_INVALID
    assert plan.blockers == (ReservationBlocker.SIZING_CONTRACT_INVALID,)


def test_5m_execution_stop_identity_must_remain_bound_to_daily_authority():
    sizing = deepcopy(_sizing_row())
    sizing["execution_stop"]["authority_signal_id"] = "other-daily"
    plan = build_execution_reservation_plan(_allocation_payload(), [sizing], _reservation_context())
    assert plan.state is ReservationPlanState.CONTRACT_INVALID
    assert plan.blockers == (ReservationBlocker.SIZING_CONTRACT_INVALID,)
