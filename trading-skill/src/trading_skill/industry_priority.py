...
# 慢变量行业质量基线。这里只描述产业结构/长期商业质量，不读取当日涨跌、资金或换手。
PROFILE_PRIORS = {
    "创新药/生物医药": 90.0,
    "造船与海工": 78.0,
    "半导体设备与材料": 91.0,
...
def _profile_name(industry_name: str, prospect_theme: str | None) -> str:
    ...
    for profile_name, tokens in PROFILE_HINTS:
        if any(token in str(industry_name or "") for token in tokens):
            return profile_name
    return profile.name
...
def _state(
    *,
    ...
    change_60d: float | None,
    change_ytd: float | None,
    long_term_prior: float,
) -> IndustryState:
    day = change_pct or 0.0
    ch60 = change_60d
    ytd = change_ytd or 0.0
...
def _pool(
    *, quality: float, market_confirmation: float, state: IndustryState, event_adjustment: float
) -> IndustryPool:
    if state in {IndustryState.OVERHEATED, IndustryState.DETERIORATING}:
        return IndustryPool.DEFERRED
    # QUALITY只能由慢变量质量门槛产生；短期市场热度永远不能把普通行业“炒成优质行业”。
    if quality >= QUALITY_FLOOR:
        return IndustryPool.QUALITY
    if quality >= EMERGING_QUALITY_FLOOR and market_confirmation >= 60 and state in {
        IndustryState.NEW,
        IndustryState.EARLY,
        IndustryState.TRENDING,
        IndustryState.MATURE,
    }:
...
def rank_industries(
    ...
    *,
    ...
    for item in normalized:
        ...
        prior = _structural_prior(item["name"], theme_name)
        ...
        market_confirmation = _weighted(((pct_p, 0.18), (flow_p, 0.27), (breadth_p, 0.25), (ch60_p, 0.30)))
        ...
        crowding = 0.0
        if ch60 is not None:
            crowding += min(10.0, max(0.0, ch60 - 35.0) * 0.30)
        crowding += min(5.0, max(0.0, ytd - 70.0) * 0.10)
        crowding += min(4.0, max(0.0, day - 5.0) * 0.8)
        ...
        state = _state(
            ...
            long_term_prior=prior,
        )
        quality = prior
        timing = 0.62 * market_confirmation + 0.30 * early + event_adj - crowding
        pool = _pool(
            quality=quality,
            market_confirmation=market_confirmation,
            state=state,
        ...
        priority = 0.67 * quality + 0.33 * timing
        if pool is IndustryPool.DEFERRED:
            priority -= 12.0
        elif pool is IndustryPool.WATCH:
            priority -= 4.0

        parts = [
            f"慢变量质量{quality:.0f}",
            f"时点{timing:.0f}",
            f"市场确认{market_confirmation:.0f}",
            f"阶段{state.value}",
            f"池{pool.value}",
        ]
        if theme_name:
            parts.append(f"长期主题:{theme_name}")
        if event_adj:
            parts.append(f"事件调整{event_adj:+.0f}")
        if crowding:
            parts.append(f"拥挤惩罚{crowding:.1f}")

        out.append(
            IndustryPriority(
                code=item["code"],
                name=item["name"],
                canonical_name=canonical_industry_name(item["name"]),
                industry_family=industry_family(item["name"]),
                state=state,
                pool=pool,
                priority_score=round(priority, 2),
                quality_score=round(quality, 2),
                timing_score=round(timing, 2),
                long_term_prior_score=round(prior, 2),
                market_confirmation_score=round(market_confirmation, 2),
                early_stage_score=round(early, 2),
                crowding_penalty=round(crowding, 2),
                event_adjustment=round(event_adj, 2),
                heat_score=round(_weighted(((pct_p, 0.35), (flow_p, 0.35), (breadth_p, 0.30))), 2),
                change_pct=item["pct"],
                change_60d=ch60,
                change_ytd=item["change_ytd"],
                main_flow_ratio=item["flow"],
                breadth=item["breadth"],
                prospect_theme=theme_name,
...
def _can_take(
    item: IndustryPriority,
    *,
    canonical_used: set[str],
    ...
    if item.canonical_name in canonical_used:
        return False
    if family_counts.get(item.industry_family, 0) >= max_per_family:
        return False
    if item.prospect_theme and theme_counts.get(item.prospect_theme, 0) >= max_per_theme:
        return False
    return True
...
def _append_selected(
    ...
    family_counts[item.industry_family] = family_counts.get(item.industry_family, 0) + 1
    if item.prospect_theme:
        theme_counts[item.prospect_theme] = theme_counts.get(item.prospect_theme, 0) + 1


def _take_pool(
    ranked: Sequence[IndustryPriority],
    *,
    pool: IndustryPool,
    quota: int,
    selected: list[IndustryPriority],
    ...
    for item in ranked:
        if sum(1 for x in selected if x.pool is pool) >= quota:
            break
        if item.pool is not pool:
            continue
        if not _can_take(
            item,
            ...
            continue
        _append_selected(item, selected, canonical_used, family_counts, theme_counts)


def select_priority_industries(
    rows: Iterable[Mapping[str, object]],
    *,
    limit: int = 28,
    event_map: Mapping[str, Iterable[Mapping[str, object]]] | None = None,
    ...
    max_per_theme: int = 2,
) -> tuple[tuple[IndustryPriority, ...], tuple[IndustryPriority, ...]]:
    """分层动态换池：QUALITY是慢变量优质行业，EMERGING/TACTICAL负责新景气和短期机会。"""
    ranked = rank_industries(rows, event_map=event_map)
    if not ranked:
        return (), ()

    quality_quota = max(1, round(limit * 0.60))
    emerging_quota = max(1, round(limit * 0.25))
    tactical_quota = max(1, limit - quality_quota - emerging_quota)
    selected: list[IndustryPriority] = []
    ...
    for pool, quota in (
        (IndustryPool.QUALITY, quality_quota),
        (IndustryPool.EMERGING, emerging_quota),
        (IndustryPool.TACTICAL, tactical_quota),
    ):
        _take_pool(
            ranked,
            pool=pool,
    ...
    if len(selected) < limit:
        for item in ranked:
            if item.pool not in {IndustryPool.QUALITY, IndustryPool.EMERGING, IndustryPool.TACTICAL}:
                continue
            if not _can_take(
                item,
                canonical_used=canonical_used,
...
ROTATION_REFRESH_POLICY = {
    "daily": "每日只重算市场确认、时点、拥挤和事件；短期涨得好只能影响EMERGING/TACTICAL，不能抬高QUALITY质量分",
    "weekly": "每周重排活动池；优质行业过热/恶化可暂退，新景气行业可进入EMERGING/TACTICAL",
    "monthly": "每月至少一次复核慢变量行业质量基线；产业格局、需求持续性或竞争格局发生变化时允许调整QUALITY名单",
    "earnings_season": "财报季按行业专属指标复核慢变量质量，后续通过质量复核模块更新基线，而不是用股价热度替代基本面",
    "event_driven": "重大事件只影响时点/战术优先级；单个政策或新闻不能直接把普通行业升级为QUALITY，也不能产生交易买点",
}
